...
public class Actor : MonoBehaviour
{
	...
	public Color BoneColor_sel = UltiDraw.Magenta;
	...
	//---------- Pairing Editor Mode; Selected Bone Coloring (only works in PairingEditor Scene) ----------
	private bool PairEditorMode = false;
	private BonePairingManager PairMgr = null;
	public string SceneName = null;
	//---------- Pairing Editor Mode; Selected Bone Coloring ----------
	...
	void Awake()
	{
		...
		{
			PairEditorMode = true;
			if (PairMgr == null) PairMgr = FindObjectOfType<BonePairingManager>();
		}
		else
		{
			PairEditorMode = false;
			PairMgr = null;
		}
		//---------- Pairing Editor Mode; Selected Bone Coloring ----------
	}
	...
	public void Draw(Color boneColor, Color jointColor, float alpha)
	{
		...
		if (DrawSkeleton)
		{
			...
			if (PairEditorMode && PairMgr != null) selBoneList = PairMgr.GetSelectedBoneList(this);
			//---------- Pairing Editor Mode; Selected Bone Coloring ----------
			...
			recursion = new Action<Bone>((bone) => {
				...
				for (int i = 0; i < bone.Childs.Length; i++)
				{
					...
					//---------- Pairing Editor Mode; Selected Bone Coloring ----------
					if (PairEditorMode && PairMgr != null)
					{
						bool changeCol = false;
						foreach (int idx in selBoneList)
						{
							if (bone.GetChild(i).Index == idx) changeCol = true;
						}

						if (changeCol) boneColor = BoneColor_sel;
						else boneColor = BoneColor;
					}
					//---------- Pairing Editor Mode; Selected Bone Coloring ----------
